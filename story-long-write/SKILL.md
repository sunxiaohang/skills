...
3. **准备层**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - 3.2 **模块召回与文风召回**：
     ...
     - (d) 输出"对标召回摘要 + 文风召回指令 + 原文锚点片段引用"（合计 ≤10 条），作为 narrative-writer 的输入
...
| 疯骚搞笑意文风 | `references/style-fengsao.md` — 都市灵异/职业志怪用，叙述有攻击性+黑色幽默 |
...
| 优先级 | 触发条件 | 动作 |
|--------|----------|------|
| 1 | 用户在 skill 参数或对话中要求「疯一点/骚一点/搞笑一点」 | **加载 `references/style-fengsao.md`**，全文风约束生效 |
| 2 | 项目 `CLAUDE.md` 中包含「疯骚搞笑」或引用 `style-fengsao` | 同上 |
| 3 | 对标书存在 `文风.md` | 加载对标文风（对标书风格模仿） |
| 4 | 以上均无 | 使用默认网文风格 |

**触发后生效的硬约束**：
- 「不是A，是B」句式：叙述中零容忍（对话例外）
- 技术名词全部人话转化
- 比喻往意想不到的方向拐
- 短句为主（15-22字），段落 ≤60 字
- 禁用万能比喻（潮水般、闪电般）
