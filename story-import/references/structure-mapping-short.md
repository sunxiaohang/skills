...
| 拆书产物 | 短篇工程文件 | 转换方式 |
|---------|------------|---------|
| 原文全文（`拆文库/{书名}/原文/`） | `{标题}/正文.md` | 单文件迁移，按 `format-and-structure.md` 规范化格式（小节标记 `###1.`、段间无空行、半角双引号对话）；原文已是成稿，**不重写内容，只规范格式** |
...
- 反转内容：{一句话描述}
- 铺垫线索：{从情节节点中的「铺垫」类节点提取，至少 3 个；不足时标 [导入反推] 待补充}
...
> 钩子字段标 `[待补充]`——导入反推无法可靠提取，留给用户或续写时填写。
...
| 规范项 | 要求 |
|--------|------|
| 小节标记 | `###1.` `###2.` `###3.`（黑岩/点众主流格式；用户指定平台时按 `format-and-structure.md` 平台覆盖表切换） |
| 段落长度 | 每段 ≤ 60 字，超出必须拆分 |
| 段间空行 | 段与段之间**不加空行**（紧密排列） |
| 对话引号 | 默认 `""` 半角双引号（知乎盐言平台改 `【】`） |
| 缩进 | 无缩进，不用全角/半角空格 |
...
- [ ] `正文.md` 单文件存在且格式符合 `format-and-structure.md`（小节标记、段长、段间无空行、引号格式）
- [ ] `设定.md` 含核心框架区块 + 对标摘要区块，且核心框架对齐 `story-short-write/SKILL.md` 模板
- [ ] `小节大纲.md` 节数与正文实际段落结构一致
- [ ] 所有 `[导入反推]` / `[待补充]` 标记已添加（不确定字段不留空）
