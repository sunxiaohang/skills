...
| `story-setup/references/agent-references/retention-writing.md` | 签约向写作（读者反应追踪、爽点时机、情绪节拍、章末钩子）时 |
...
> 签约规则由 `.claude/rules/story-signing.md` 自动加载并优先执行。详细技法见 `story-setup/references/agent-references/retention-writing.md`。

**核心原则**：平台签约评估的不是"写得好不好"，而是"能不能留住读者"。每次写作时，以下约束自动生效：

- **开篇铁律**：前 50 字禁止静态描写。前 300 字必须让读者知道"主角是谁、在什么处境、有什么即时的压力"。前 300 字内出现至少一个平台热词标签（系统/穿越/重生/种田/饥荒/乱世/商城）。
- **爽点时机**：第一个情绪高点在前 800 字内。爽点在 1500 字之后 → 读者的留存已经掉光了。
- **情绪节拍**：情绪不能单方向超过 800 字。压抑超过 800 字给温暖，爽超过 800 字埋隐患。每次转折必须有具体事件驱动。
- **读者反应追踪**：每 ~200 字问自己——"读者脑子里现在有没有一个必须被回答的问题？"连续 300 字无新问题 → 立刻插入冲突/悬念/反常细节。
- **章末钩子强制五选一**：只允许倒计时/直接威胁/承诺兑现/身份揭示/两难抉择。严禁留白/氛围/抒情收尾。
- **主角主动性**：第一章至少 2 个主动选择。每章至少一次"读者没想到但觉得合理"的动作。
- **出场人物**：第一章有名有姓有台词 ≤ 5 个。
- **对话潜台词**：超过 3 轮对话必须有一个角色在说"不是他真正想说的"。

**与文风的关系**：签约结构约束是硬框架，文风是你填充框架的方式。结构确保留存，文风确保辨识度。两者不冲突——结构不好的文风 = 没人读的好文章。
...
**⚠️ 每章生成完毕、去AI味之后，必须执行以下三道自检。任何一道不通过 = 未完成，重写到通过。**

**自检 1 — 前 300 字独立理解**：把前 300 字单独拿出来，一个没读过这本书的人能否说出"主角是谁、在干嘛、有什么麻烦"？不能 → 开篇重写。

**自检 2 — 章末追读钩子**：章末最后一句话之后，读者脑子里是否有一个**今晚就必须被回答**的问题？检查方法：把章末钩子单独读一遍——如果读者可以说"明天再看也行"，钩子不及格 → 章末重写。

**自检 3 — 主角主动性**：这一章里，主角主动做的选择 ≥ 2 个吗？（第一章硬性要求 ≥ 2；后续章节 ≥ 1）。注意："拒绝"不算选择，"在两个选项中选了其中一个"才算。

**自检结果标记**：三道全过后，在章节文件末尾加 HTML 注释标记（不影响正文渲染）：
```
<!-- signing-check: D1前300字=PASS D2章末钩子=PASS D3主角主动=PASS -->
```
